namespace YS.Application.IoT.CommandHandler.Downlink
{
    ...
    /// </summary>
    public class DownlinkCommand6200 : CommandHandlerBase<DownlinkCommand6200>
    {
        /// <summary>
        /// 初始化一个新的 DownlinkCommand6200 实例。
        /// </summary>
        /// <param name="logger">日志记录器。</param>
        /// <param name="scopeFactory">服务作用域工厂，用于解析依赖项。</param>
        public DownlinkCommand6200(ILogger<DownlinkCommand6200> logger, IServiceScopeFactory scopeFactory)
            : base(logger, scopeFactory)
        {
        }

        /// <summary>
        /// 处理事件标识为 "6200" 和 "6200:V1" 的下行指令。
        /// </summary>
        /// <param name="context">事件执行上下文，包含消息体等信息。</param>
        /// <returns>异步操作任务。</returns>
        [EventSubscribe("6200")]
        [EventSubscribe("6200:V1")]
        public override async Task HandleAsync(EventHandlerExecutingContext context)
        {
            ...
            // 解析下行请求数据
            var request = payload.Body.FromJson<DownlinkEntity6200.Response>();
            if (request == null)
            {
            ...
            // 调用 gRPC 服务处理下行指令 6200
            var result = await GrpcWrapp.Instance.GrpcCommandService.Downlink6200HandleAsync(request);
