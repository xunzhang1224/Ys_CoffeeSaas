...
namespace YS.CoffeeMachine.Platform.API.Application.PlatformCommandHandlers.ApplicationCommandHandlers.MenuCommandHandlers
{
    ...
    public class P_DeleteMenuCommandHandler(IPApplicationMenuRepository repository) : ICommandHandler<P_DeleteMenuCommand, bool>
    {
        /// <summary>
        /// 删除菜单
        /// </summary>
        public async Task<bool> Handle(P_DeleteMenuCommand request, CancellationToken cancellationToken)
        {
            if (request.id <= 0)
