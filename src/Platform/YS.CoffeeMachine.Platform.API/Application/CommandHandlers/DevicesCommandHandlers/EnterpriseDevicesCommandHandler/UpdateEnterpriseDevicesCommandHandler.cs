...
namespace YS.CoffeeMachine.Platform.API.Application.CommandHandlers.DevicesCommandHandlers.EnterpriseDevicesCommandHandler
{
    ...
    public class UpdateEnterpriseDevicesCommandHandler(IPEnterpriseDevicesRepository repository, CoffeeMachinePlatformDbContext context) : ICommandHandler<UpdateEnterpriseDevicesCommand, bool>
    {
        /// <summary>
        /// 编辑设备分配
        /// </summary>
        public async Task<bool> Handle(UpdateEnterpriseDevicesCommand request, CancellationToken cancellationToken)
        {
            ...
            //重新分配设备
            info.Update(request.enterpriseId, request.allocationEnum, request.recyclingTime, allocateTime);
