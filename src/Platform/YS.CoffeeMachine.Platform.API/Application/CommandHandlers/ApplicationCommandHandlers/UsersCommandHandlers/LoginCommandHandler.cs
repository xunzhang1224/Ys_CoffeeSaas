...
namespace YS.CoffeeMachine.Platform.API.Application.CommandHandlers.ApplicationCommandHandlers.UsersCommandHandlers
{
    ...
    public class LoginCommandHandler(CoffeeMachinePlatformDbContext context, IPasswordHasher passwordHasher, IJwtTokenService jwtTokenService) : ICommandHandler<LoginCommand, LoginResponseDto>
    {
        /// <summary>
        /// 商户登录
        /// </summary>
        ...
        {
            var userInfo = await context.ApplicationUser.AsNoTracking().Include(i => i.ApplicationUserRoles).ThenInclude(ti => ti.Role).FirstOrDefaultAsync(w => w.Account == request.account && !w.IsDelete);
            ...
            //var menuTree = await queries.GetUserMenuTreeAsync(userInfo.Id);
