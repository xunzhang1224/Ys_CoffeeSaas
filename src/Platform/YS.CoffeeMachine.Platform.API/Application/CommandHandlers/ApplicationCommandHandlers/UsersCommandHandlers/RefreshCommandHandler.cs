...
namespace YS.CoffeeMachine.Platform.API.Application.CommandHandlers.ApplicationCommandHandlers.UsersCommandHandlers
{
    ...
    public class RefreshCommandHandler(/*IApplicationMenuQueries queries,*/ CoffeeMachinePlatformDbContext context, IJwtTokenService jwtTokenService) : ICommandHandler<RefreshCommand, LoginResponseDto>
    {
        /// <summary>
        /// 刷新Token
        /// </summary>
        public async Task<LoginResponseDto> Handle(RefreshCommand request, CancellationToken cancellationToken)
        {
            ...
            var userInfo = await context.ApplicationUser.AsNoTracking().FirstAsync(w => w.Id == Convert.ToInt64(uid));
