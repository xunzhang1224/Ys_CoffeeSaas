...
namespace YS.CoffeeMachine.Platform.API.Application.CommandHandlers.ApplicationCommandHandlers.RolesCommandHandlers
{
    ...
    public class UpdateApplicationRoleCommandHandler(CoffeeMachinePlatformDbContext context, IRedisClient _redisClient) : ICommandHandler<UpdateApplicationRoleCommand, bool>
    {
        ...
        {
            var exists = await context.ApplicationRole.AsQueryable().AnyAsync(w => w.Name == request.name && w.Id != request.id);
            if (exists)
                throw ExceptionHelper.AppFriendly("角色名称重复");

            var info = await context.ApplicationRole.AsNoTracking().Include(i => i.ApplicationRoleMenus).FirstAsync(w => w.Id == request.id);
            if (info == null)
            ...
            if (request.menuIds != null && !request.menuIds.OrderBy(o => o).SequenceEqual(info.ApplicationRoleMenus.Select(s => s.MenuId).OrderBy(o => o)))
            {
                ...
                info.UpdateRoleMenuIds(request.menuIds.ToList());
            }
            //修改所属系统类型
