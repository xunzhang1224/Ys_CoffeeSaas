...
namespace YS.CoffeeMachine.API.Queries.FileResourceQueries
{
    ...
    public class FileCenterQuerie(CoffeeMachineDbContext _context) : IFileCenterQuerie
    {
        /// <summary>
        /// 查询文件中心列表
        /// </summary>
        ...
        public async Task<PagedResultDto<FileCenter>> GetFileCenter(FileCenterInput input)
        {
            return await _context.FileCenter.AsQueryable()
                ...
                .WhereIf(input?.SysMenuType == null, x => x.SysMenuType == SysMenuTypeEnum.Merchant || x.SysMenuType == SysMenuTypeEnum.H5)
