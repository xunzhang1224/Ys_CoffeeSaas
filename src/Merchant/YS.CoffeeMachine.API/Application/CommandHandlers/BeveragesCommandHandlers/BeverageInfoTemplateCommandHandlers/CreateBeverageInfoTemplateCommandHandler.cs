...
namespace YS.CoffeeMachine.API.Application.CommandHandlers.BeveragesCommandHandlers.BeverageInfoTemplateCommandHandlers
{
    ...
    public class CreateBeverageInfoTemplateCommandHandler(IBeverageInfoTemplateRepository repository) : ICommandHandler<CreateBeverageInfoTemplateCommand, bool>
    {
        /// <summary>
        /// 创建饮品库模型
        /// </summary>
        ...
        {
            var info = new BeverageInfoTemplate(request.enterpriseInfoId, request.deviceModelId, request.name, request.beverageIcon, request.temperature, request.remarks, request.productionForecast, request.forecastQuantity, request.displayStatus, request.sellStradgy, categoryIds: request.productCategoryIds);
            //设置Code信息
