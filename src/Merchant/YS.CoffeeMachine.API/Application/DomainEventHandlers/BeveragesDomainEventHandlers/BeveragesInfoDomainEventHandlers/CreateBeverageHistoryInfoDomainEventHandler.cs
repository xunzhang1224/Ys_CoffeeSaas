...
namespace YS.CoffeeMachine.API.Application.DomainEventHandlers.BeveragesDomainEventHandlers.BeveragesInfoDomainEventHandlers
{
    /// <summary>
    /// 创建饮品编辑历史事件
    /// </summary>
    ...
    public class CreateBeverageHistoryInfoDomainEventHandler(IMediator mediator, IMapper mapper) : IDomainEventHandler<CreateBeverageHistoryInfoDomainEvent>
    {
        ...
        public async Task Handle(CreateBeverageHistoryInfoDomainEvent notification, CancellationToken cancellationToken)
        {
            ...
            //初始化饮品历史信息
            var historyInfo = new BeverageInfo(info.DeviceId, info.Name, info.BeverageIcon, info.Temperature, info.Remarks, info.ProductionForecast, info.ForecastQuantity, info.DisplayStatus, info.IsDefault, null, info.SellStradgy);
            historyInfo.AddCodeNotId(info.Code);
            //绑定配方
            historyInfo.AddRangeFormulaInfos(info.FormulaInfos);
            //绑定主饮品
            //historyInfo.BindParentId(info.TransId);
            //对象转换后调用创建饮品信息命令
