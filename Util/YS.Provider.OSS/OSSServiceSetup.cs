namespace YS.Provider.OSS
{
    ...
    using YS.Provider.OSS.Providers;

    /// <summary>
    /// OSSServiceSetup
    /// </summary>
    public static class OSSServiceSetup
    {
        /// <summary>
        /// 从配置文件中加载默认配置
        /// </summary>
        ...
        public static IServiceCollection AddOSSService(this IServiceCollection services, string name, string key)
        {
            using (ServiceProvider provider = services.BuildServiceProvider())
            {
                ...
                return services.AddOSSService(name, o =>
                {
                    ...
                    o.CDNAddress = options.CDNAddress;
                });
            }
        }
        ...
        public static IServiceCollection AddOSSService(this IServiceCollection services, string name, Action<OSSOptions> option)
        {
            ...
            //
            services.TryAddSingleton(sp => sp.GetRequiredService<IOSSServiceFactory>().Create(name));
            return services;
        }
    }
}
